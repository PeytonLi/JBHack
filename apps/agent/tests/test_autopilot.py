...
def app_and_patches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    async def fake_open_pr(*, token, repo, incident_id, analysis, relative_path, updated_file_content) -> PullRequestResult:
        return PullRequestResult(pr_url="https://github.com/acme/demo/pull/1", pr_number=1, branch="secureloop/inc-1")
